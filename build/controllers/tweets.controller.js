...
const tweetList = async (req, res, next) => {
    try {
        const tweets = await (0, tweets_queries_1.getCurrentUserAndFollowersTweets)(req.user);
...
const tweetCreate = async (req, res) => {
    try {
        const body = req.body;
        await (0, tweets_queries_1.createTweet)({ ...body, author: req.user._id });
        res.redirect("/tweets");
    }
    catch (e) {
...
const tweetEdit = async (req, res, next) => {
    try {
        const tweetId = req.params.tweetId;
        const tweet = await (0, tweets_queries_1.getTweet)(tweetId);
        res.render("tweets/tweet-form", {
            tweet,
            isAuthenticated: req.isAuthenticated(),
            currentUser: req.user,
...
const tweetUpdate = async (req, res) => {
    ...
    try {
        const body = req.body;
        await (0, tweets_queries_1.updateTweet)(tweetId, body);
        res.redirect("/tweets");
    }
    catch (e) {
        const errors = Object.keys(e.errors).map((key) => e.errors[key].message);
